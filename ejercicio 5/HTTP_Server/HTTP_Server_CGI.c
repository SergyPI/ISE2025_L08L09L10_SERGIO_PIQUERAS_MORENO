...
// Local variables.
static uint8_t P2;
static uint8_t rgb; // para el rgb
...
void netCGI_ProcessData (uint8_t code, const char *data, uint32_t len) {
  ...
  P2 = 0;
	rgb=0;//descomento esta linea si quiero que inicialmente el rgb este encendido, que seria estar de color blanco
	      //(los 3 colores low)
 CheckBox=P2|rgb;
 LEDrun = true;
  if (len == 0) {
...
uint32_t netCGI_Script (const char *env, char *buf, uint32_t buflen, uint32_t *pcgi) {
  ...
  switch (env[0]) {
    ...
    case 'b':
      ...
      id = (uint8_t)(1U << id);
      len = (uint32_t)sprintf (buf, &env[4], ((P2|rgb) & id) ? "checked" : ""); //ponemos el rgb para que al actualizar le html se quede el tick del rgb
      break;
    ...
    case 'g':
      // AD Input from 'ad.cgi'
        ...
        case '2':  // POT 2 (PC3)
            adv = AD_in(1);
            len = (uint32_t)sprintf(buf, &env[4], (double)((float)adv * 3.3) / 4096);
            break;
        case '3':
          adv = (adv * 100) / 4096;
          len = (uint32_t)sprintf (buf, &env[4], adv);
