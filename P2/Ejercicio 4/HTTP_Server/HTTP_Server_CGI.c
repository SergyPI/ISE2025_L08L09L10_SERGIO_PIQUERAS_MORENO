...
/*necesario hacer reset aqui ya que permite, se selecciono de nuevo
  el mismo led, poder apagarlo.Quitar esta linea hace que tenga que clicar 4 veces
  para apagarlos*/
	reset_leds(); 

  P2 = 0; //tambien necesaria esta linea para poder eliminar el tick cuando ya está y se clica
  //rgb=1; //descomento esta linea si quiero que inicialmente el rgb este encendido, que seria estar de color blanco(los 3 colores HIGH)
  ...
  if (len == 0) {
    ...
    //LED_SetOut (P2);
    //salida_leds(P2,rgb);
    reset_leds();//reseteo todos los leds antes de entrar en aplicacion LEDS
    return;
  }
  passw[0] = 1;
  do {
    // Parse all parameters
    data = netCGI_GetEnvVar (data, var, sizeof (var));//data es mi puntero de environment variable.Var es mi buffer de salida para escribir mi variable convertida.
    //esta funcion devuelve otro puntero a variables todavía no convertidas.
    if (var[0] != 0) {
      // First character is non-null, string exists
      /*compara el string var con browser mediante la funcion strcmp
        y devuelve 0 si son iguales, o lo que en este caso es lo mismo, si el string existe, devolviendo un 0*/
      if (strcmp (var, "led1=on") == 0) {
        P2 |= 0x02;
        HAL_GPIO_TogglePin(GPIOB,GPIO_PIN_0);
      }//tanto led0,led1,led2 y browser están en mi fichero leds.cgi para ser procesados
        ...
        HAL_GPIO_TogglePin(GPIOB,GPIO_PIN_14);
            ...
            else if (strcmp (var, "blue=on") == 0){
         P2 |= 0x10;
         HAL_GPIO_TogglePin(GPIOD,GPIO_PIN_11);
        //rgb |= 0x10;
      }
      else if (strcmp (var, "green=on") == 0) { 
        P2 |= 0x20;
        HAL_GPIO_TogglePin(GPIOD,GPIO_PIN_12);
        //rgb |= 0x20;
      }
            else if (strcmp (var, "red=on") == 0) {
         P2 |= 0x40;
         HAL_GPIO_TogglePin(GPIOD,GPIO_PIN_13);
        //rgb |= 0x40;
      }
      else if (strcmp (var, "ctrl=Browser") == 0) {
        LEDrun = false;//booleano que me para el barrido automático
        
      }
      else if ((strncmp (var, "pw0=", 4) == 0) ||//pw0 y pw2 están en el fichero system.cgi y solo valen para contraseña.
               (strncmp (var, "pw2=", 4) == 0)) {
        ...
        // LCD Module line 1 text. Recibe la info escrita en la web y por señal lo manda a la funcion del hilo LCD
        strcpy (lcd_text[0], var+5);
        osThreadFlagsSet (TID_Display, LCD); 
      }
      else if (strncmp (var, "lcd2=", 5) == 0) {
        // LCD Module line 2 text. Recibe la info escrita en la web y por señal lo manda a la funcion del hilo LCD
    ...
    //salida_leds(P2,rgb); 
  //CheckBox=P2|rgb;//concateno los leds con los rgbs y lo paso a checkboxes 
}
...
uint32_t netCGI_Script (const char *env, char *buf, uint32_t buflen, uint32_t *pcgi) {
  ...
  switch (env[0]) {
    ...
    case 'b':
      ...
      id = (uint8_t)(1 << id);
      len = (uint32_t)sprintf (buf, &env[4], (/*CheckBox*/P2 & id) ? "checked" : "");
      break;
    ...
    case 'f':
      // LCD Module control from 'lcd.cgi'  aquí envia la info a la web de lo escrito
      switch (env[2]) {
        case '1':
    ...
    case 'g':
      // AD Input from 'ad.cgi' Aqui se envia la info a la web de mi potencionmetro a la hora de manejarlo
      switch (env[2]) {
        case '1': //digital value
          adv = AD_in (0);
      ...
      switch (env[2]) {
        case '1'://fecha y hora
           timedate=Obtener_FechaYHora();
           char buffer2[60];
          strcat(strcpy(buffer2, timedate.time),timedate.date);
